        ...
        public int GetMaximumPrice()
        {
            using (var context = new CBContext())
            {
               // return (int)(context.Products.Max(x => x.Price));
                var maxPriceProduct = context.Products.OrderByDescending(x => x.Price).FirstOrDefault();
                if (maxPriceProduct != null)
                {
                    return (int)maxPriceProduct.Price;
                }
                else
                {
                    // Koi product nahi hai, aap yahan kuch default value ya handle karne ka logic add kar sakte hain.
                    return 0; // Ya phir kuch aur value jo aapke use case ke liye theek ho.
                }
            }
